    ...
    pub fn defaulting(&mut self, node: &Rc<RefCell<Node>>, parent_style: Option<ComputedStyle>) {
        ...
        if let Some(parent_style) = parent_style {
            ...
            if self.color.is_none() && parent_style.color() != Color::black() {
                // 親要素のcolorプロパティがブラウザによって定められた初期値でない場合
                // ユーザーによって設定されている値なので親要素の値を継承する
...
impl Color {
    ...
    pub fn white() -> Self {
        Self { name: Some("white".to_string()), code: "#ffffff".to_string() }
    }

    pub fn black() -> Self {
        Self { name: Some("black".to_string()), code: "#000000".to_string() }
    }
...
pub enum DisplayType {
    Block, // https://www.w3.org/TR/css-display-3/#valdef-display-block
    Inline, // https://www.w3.org/TR/css-display-3/#valdef-display-inline
    DisplayNone, // https://www.w3.org/TR/css-display-3/#valdef-display-none
}
